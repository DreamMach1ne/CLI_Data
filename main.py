...
def main():
    ...
    parser.add_argument('--gcs_bucket', type=str, default=None, help='Google Cloud Storage bucket to upload files to (optional)')
    ...
    # Create a dictionary to hold optional arguments
    optional_args = {}
    if args.gcs_bucket:
        optional_args['gcs_bucket'] = args.gcs_bucket


    if args.source == 'bnci':
        success = download_from_bnci("http://bnci-horizon-2020.eu/database/data-sets", destination_folder, **optional_args)
    elif args.source == 'figshare':
        success = download_from_figshare("https://api.figshare.com/v2/collections/3917698/articles", destination_folder, **optional_args)
    elif args.source == 'mindbigdata':
        success = download_from_mindnet(destination_folder, **optional_args)
    elif args.source == 'gigadb':
        success = download_from_gigadb("http://gigadb.org/dataset/view/id/100295", destination_folder, **optional_args)
        ...
        success = download_direct_links(direct_links, destination_folder, **optional_args)
    elif args.source == 'kaggle':
        kaggle_commands = [
            'kaggle competitions download -c grasp-and-lift-eeg-detection',
            'kaggle competitions download -c inria-bci-challenge'
        ]
        success = download_kaggle_datasets(kaggle_commands, destination_folder, **optional_args)
