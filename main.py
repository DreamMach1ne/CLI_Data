...
def main():
    ...
    for source in sources:
        try:
            ...
            if success:
                    print(f"[SUCCESS] Completed downloading all files from {source}.")
            else:
                print(f"[FAILURE] Failed to download all files from {source}.")
                failed_sources.append(source)

        except Exception as e:
            print(f"[ERROR] An exception occurred while downloading from {source}: {e}")
            failed_sources.append(source)

        if failed_sources:
            print(f"[SUMMARY] Failed to download from the following sources: {', '.join(failed_sources)}")
            # Optionally, write the failed_sources list to a log file
            with open("failed_sources.log", "w") as f:
